...
(def basic-handlers
  {:get
   (fn [tx {:keys [key-space key]}]
     (cs/get-val tx key-space key false false))
   :get-keys
   ...
   (fn [tx {:keys [key-space key value]}]
     (cs/set-val tx key-space key value))
   :del
   (fn [tx {:keys [key-space key]}]
   ...
   (fn [tx {:keys [key-space key value]
            :or {value 1}}]
     (let [v (or (cs/get-val tx key-space key false false) 0)
           v' (+ v value)]
       (cs/set-val tx key-space key v')
       v'))
   :swap
   (fn [tx {:keys [key-space key key']}]
     (let [v (cs/get-val tx key-space key false false)
           v' (cs/get-val tx key-space key' false false)]
       (cs/set-val tx key-space key v')
       (cs/set-val tx key-space key' v)))})
