...
(defn create-transport
  ...
  (let [opts {:epoll? true
              :raw-stream? false
              :bootstrap-transform (partial bootstrap-tcp tcp)}
        ...
        (fn [remote-endpoint connection]
          (flow
            ...
            (ms/on-closed connection
                          (fn []
                            (del-connection! remote-endpoint connection)
                            (ms/put! events
                                     (->EndpointDisconnected remote-endpoint))))
            (ms/connect
             (->> connection
                  (ms/filter (comp not #{:ping} :type))
                  (ms/map (partial ->Received remote-endpoint)))
             events
             {:downstream? false
              :upstream? true})
            (spawn-pinger ping-period connection)
            true))
        server
        (start-server
         (fn [connection _]
           ...
           (flow
             ...
             (=> (client
                  (assoc opts
                         ...
                         :bootstrap-transform (partial client-bootstrap tcp)))
...
(defn- spawn-pinger [period connection]
  (md/loop []
...
(comment
  ...
  (do
    (def t1 (create-transport ex codec/message {:host "127.0.0.1" :port 9101}))
    (def t2 (create-transport ex codec/message {:host "127.0.0.1" :port 9102}))
    (ms/consume (partial println "T1") (events t1))
    (ms/consume (partial println "T2") (events t2)))
  (do
    (connect-to t1 (u/socket-address "127.0.0.1" 9102))
    (connect-to t2 (u/socket-address "127.0.0.1" 9101)))
  (do
    (send-to t2 (u/socket-address "127.0.0.1" 9101)
             {:type :letter
              :sender 666
              :receiver 777
              :body "Foo Bar Baz"}))
